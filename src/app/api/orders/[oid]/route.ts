...
function formatVia(via: string | undefined, method: string | undefined) {
  ...
  if (method === "upi") {
    ...
    return via.split(",")[1].toUpperCase() + " " + "****" + " " + via.split(",")[0]
...
function formatMethod(method: string | undefined) {
  ...
  if (method === "upi") {
    ...
    const capitalizeName = name.substring(0, 1).toUpperCase() + name.substring(1, name.length)
    const capitalizeCard = card.substring(0, 1).toUpperCase() + card.substring(1, card.length)
    return capitalizeName + " " + capitalizeCard
  } else if (method === "wallet") {
    return method.substring(0, 1).toUpperCase() + method.substring(1, method.length)
  } else if (method === "netbanking") {
    const net = method.substring(0, 3)
    const banking = method.substring(3, method.length)
    const capitalizeNet = net.substring(0, 1).toUpperCase() + net.substring(1, net.length)
    const capitalizeBanking = banking.substring(0, 1).toUpperCase() + banking.substring(1, banking.length)
    return capitalizeNet + " " + capitalizeBanking
  }
  return null
}

export async function GET(req: NextRequest, { params }: { params: { oid: string } }) {
  try {
    ...
    if (!orderId) return error400("Missing Order Id parameter", { order: null })
    ...
    if (!address) return error400("Invalid Delivery Address Id", { order: null })
    ...
    const orderItems = order.orderItems.map((orderItem) => {
      ...
      return {
        title: orderItem.product?.title || "Product",
        ...
        basePrice: orderItem.basePrice,
        offerPrice: orderItem.offerPrice,
        imageUrl: getImageThumbnail({ images: orderItem.product?.images || [] }, orderItem.color),
      }
    })
