...
async function findParentAndEndChildIds(
  data: Category[],
  categoryId?: string
): Promise<string[]> {
  const parentIds: Category[] = [];
  const endChildIds: string[] = [];

  // Recursive function to find all parent categories
  function findParents(categoryId?: string) {
    const parents = data.filter((item) => item.parentId === categoryId);
    parentIds.push(...parents);

    // If no parents are found and the category exists, add it to endChildIds
    if (!parentIds.length && data.some((item) => item.id === categoryId)) {
      endChildIds.push(data.find((item) => item.id === categoryId)!.id);
    }

    // Recursively find parents for each parent category
    parents.forEach((parent) => findParents(parent.id));
  }

  // Start finding parents from the given categoryId
  findParents(categoryId);

  // Find all end-child categories (categories with no children)
